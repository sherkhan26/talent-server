...
function makeClientFromData(data) {
  ...
  const client = {
    ...
    parentSurname: asString(data.parentSurname),
    parentLastName: asString(data.parentLastName),
    talent: asString(data.talent),
    age: asString(data.age),
    company: asString(data.company),
    dzo: asString(data.dzo),
    userImg: asString(data.userImg),
    userVideo: asString(data.userVideo),
  ...
  if (!client.parentName) errors.push({ field: 'parentName', message: 'Не указано имя Родителя' })
  if (!client.parentSurname) errors.push({ field: 'parentSurname', message: 'Не указано имя Родителя' })
  if (!client.talent) errors.push({ field: 'talent', message: 'Не указан Талант' });
  if (!client.age) errors.push({ field: 'age', message: 'Не указан Возраст' });
  if (!client.company) errors.push({ field: 'age', message: 'Не указана Компания'})
  if (!client.dzo) errors.push({ field: 'dzo', message: 'Не указан Дзо' });
  if (!client.userImg) errors.push({ field: 'userImg', message: 'Не загружена фото'})
  if (!client.userVideo) errors.push({ field: 'userVideo', message: 'Не загружена видео'})
